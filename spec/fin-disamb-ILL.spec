...
#
ILL-EXCL: ^((aasia|abihun|baani|ben-haanan|daan|ester|geehasi|guuni|
...
# 6 N-COM: GEN (V-front, -n): 
  ...
  vuoriseläntee)(n|(POSS-SFX-V)))(CLT-SFX?)|
#
...
# 31 N-COM: SG-INE (V-front, -ssä): 
  ...
  ylpeyde|ylvästely|ymmärrykse)ssä)(POSS-SFX?CLT-SFX?)|
#
...
# 35 N-COM: SG-ADE (V-front, -llä): 
  ...
  ääne|hedelmä|isä|keihää|leiki|pöydä|((selko)?selä)|synni|tie|työ|
...
# 39 N-COM: SG-ELA (V-front, -stä):   
  ((ääne|hälinä|elime|hedelmä|henge|henkäykse|ikävöimise|isä|
...
# 47 N-COM: SG-ALL (V-front, -lle): 
  ((ääne|äidi|hädä|isä|isännä|käde|kämmene|kiele|lähettäjä|lähimmäise|miehe|
...
# 54 A: SG-NOM (V-back, positive):
  (nopea|viisaa)(POSS-SFX-V?CLT-SFX?)|
#
...
#  toista|toisessa|toiselta|toisessa|toisesta|toiselle|toiselleen|
#  toisia|toisissa|toisista|toisille|toisilta|toisilleen|
...
# 98 V: INF-IV, SG-PTV (V-front, -minen: -mista-tä): 
  ...
  vähene|väisty|vihkiyty|ylty)mis)tä)
  (POSS-SFX?CLT-SFX?)|
#
...
# 120 V-ACT, IND, ACT-PRES (V-back):
  ...
  tehoa|tempaa|todista|tule|turvaa|uhkaa|uhraa|vainoa|vastaa|veisaa|
...
# 147 V: PASS-PRES (V-back, -ta-an): 
  ...
  varaste|varuste|vasote|vasta|vastaampte|vavis|veisa|verho|viska|vuodate|
...
# 167 Adverbs, conjunctions:
  ...
  yliyltään|yltyleensä|ylleen|jos|sela)(POSS-SFX?CLT-SFX?)|
#
...
#
#
#^(asuinsijaasi|elämäänsä|elämäänsäkin|etsikkoaikaasi|
#  haavaasi|hautaan|härkäänsä|herraamme|herraansa|herraani|herraasi|
#  herraansa|ikääni|ikäänsä|jalkaasi|jumalaamme|jumalaani|jumalaasi|
#  jumalaanne|kanssapalvelijaasi|kaulaanne|kaulaansa|kärsimäänsä|
#  kuokkaansa|kuolemaansa|leposijaansa|lihaamme|lihaani|lihaanne|
#  lihaansa|lihaasi|lonkkaansa|luojaansa|määrään|määräänne|majaasi|
#  matkaanne|matkaansa|miekkaani|miekkaansa|nälkäänsä|niskaansa|
#  paikkaasi|palkkaani|palkkaansa|palvelustehtäväänsä|perhekuntaansa|
#  perintöosaansa|perintöosaasi|pesääsi|rintaani|selkääni|selkääsi|
#  silmääni|tähtijumalaanne|tekemäänsä|valtaansa|valtankuntaani|
#  vastustajaansa|vatsaani|vatsaansa|vesiastiaasi|viinitarhaani|
#  viinitarhaasi|virkaamme|virkaani|virkaansa|voimaasi)
# 
# Muistiin: e-loppuiset substantiivit: katse: katseensa
# Entä: kauriin: kauriiseensa, kauriini: kauriiseeni
# Tarkasta: kolmanne|neljännekse|lupaukse|pahennukse|parannukse)en)|
# ILL-EXCL-PCPL-PRES: 
# STEM+vi+naan|vi+nään: joutuvinaan, kieltävinään
# ILL-EXCL-PCPL-PAST: 
# STEM+ttu+aan|tty+ään: suostutettuaan, luettuaan, pyöritettyään
#
